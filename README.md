...
このハンズオンでは、まずローカルDocker環境で小さなアプリを動かし、OpenTelemetry Collectorを通してトレースとメトリクスを観察します。Section 2 から Section 7 まではAWSアカウントなしで進められます。Section 9 では任意の発展ハンズオンとして、AWSアカウントを使ってLambda、ECS/Fargate、ADOT、CloudWatch、X-Rayの接続を確認します。
...
Section 2 から Section 7 までのハンズオンはローカル環境だけで完結します。AWS、Google Cloud、Azureなどのクラウドリソースは作成しません。Section 9 のAWS連携ハンズオンを実行する場合だけ、AWSリソースを短時間作成します。
...
## Section 9 AWS integration hands-on

Section 9 では、AWS連携の発展ハンズオンとして Lambda + ADOT Layer と ECS/Fargate + ADOT Collector sidecar を確認します。

扱う内容:

- Lambda関数にADOT Layerを設定する
- Lambda実行ログとX-Ray trace summaryを確認する
- ECS/FargateタスクでアプリコンテナとADOT Collector sidecarを同じタスク内で動かす
- telemetrygenからCollectorへOTLP traceを送り、X-Ray trace summaryを確認する
- 作成したAWSリソースをcleanupスクリプトで削除する
...
Instructor verification evidence for the Section 9 AWS integration flow is stored in:

```text
verification/2026-06-20-section9-aws/
```
